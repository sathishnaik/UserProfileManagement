...
public class LoginValidator implements Validator {
	...
	private void validateLogin(User formUserObj, Errors errors) {
	    ...
	    if(!errors.hasErrors()){
	    	...
	    	if(!isValidUser){
	    		errors.rejectValue("password", "LoginValidator.password.wrong");
	    	}
	    	if(userFromDB!=null && userFromDB.getOtp()!=null){
	    		...
	    		if(timeElapsed>100000){
	    			errors.rejectValue("otp.otpId", "LoginValidator.otp.otpId.expired");
	    			//remove expired OTP
	    			otpDelegate.removeOtpById(userFromDB.getOtp().getOtpId());
	    		}
